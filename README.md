...
Upload a JPEG, PNG, WebP, or TIFF image to resize it with Sharp, convert it to WebP, and inspect the returned metadata. HEIC/HEIF uploads are still rejected: the API returns an error before processing so unsupported-format cases can be surfaced to Sentry with upload context attached.
