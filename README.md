...
Upload a JPEG, PNG, WebP, or TIFF image to resize it with Sharp, convert it to WebP, and inspect the returned metadata. HEIC/HEIF files can be chosen in the file picker but are not processed by Sharp in this demo; the API returns a clear error before decoding.
