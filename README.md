...
Upload a JPEG, PNG, WebP, or TIFF image to resize it with Sharp, convert it to WebP, and inspect the returned metadata. HEIC/HEIF can be chosen in the file picker but are not processed by the API (unsupported MIME types return a thrown error with upload context for Sentry).

## Sentry demo scenario (HEIC / HEIF)
