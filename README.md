...
Upload a JPEG, PNG, WebP, or TIFF image to resize it with Sharp, convert it to WebP, and inspect the returned metadata. HEIC / HEIF remain intentionally unsupported in the API (validation fails before processing), which surfaces the exception to Sentry with upload context attached.

## Sentry demo scenario

1. Start the app locally.
2. Open the upload page.
3. Drop or select a `.heic` or `.heif` file (TIFF is processed normally).
