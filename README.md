...
Upload a JPEG, PNG, WebP, or TIFF image to resize it with Sharp, convert it to WebP, and inspect the returned metadata. Unsupported upload types return a structured client error instead of raising an exception.

## Upload validation

The file picker advertises the same formats the API can process. Direct API calls with unsupported MIME types receive a `415` JSON response.
